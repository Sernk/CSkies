            ...
            if (dotiles)
            {
                ...
                for (int x1 = radiusLeft; x1 <= radiusRight; x1++)
                {
                    for (int y1 = radiusUp; y1 <= radiusDown; y1++)
                    {
                        ...
                        float distY = Math.Abs(y1 - position.Y / 16f);
                        double dist = Math.Sqrt(distX * distX + distY * distY);
                        if (dist < explosionIntensity && Main.tile[x1, y1] != null && Main.tile[x1, y1].WallType == 0)
                        {
                            updateWalls = true;
                            break;
                        }
                ...
                for (int x2 = radiusLeft; x2 <= radiusRight; x2++)
                {
                    for (int y2 = radiusUp; y2 <= radiusDown; y2++)
                    {
                        ...
                        if (dist < explosionIntensity)
                        {
                            ...
                            {
                                if (BaseUtility.InArray(BaseConstants.TILEIDS_DUNGEONSTRICT, Main.tile[x2, y2].TileType) || Main.tile[x2, y2].TileType == 21 || Main.tile[x2, y2].TileType == 26 || Main.tile[x2, y2].TileType == 107 || Main.tile[x2, y2].TileType == 108 || Main.tile[x2, y2].TileType == 111)
                                {
                                    canExplode = false;
                                }
                                ...
                                if (canExplode)
                                {
                                    ...
                                    if (sync && !Main.tile[x2, y2].HasTile && Main.netMode != NetmodeID.SinglePlayer) { NetMessage.SendData(BaseConstants.NET_TILE_UPDATE, -1, -1, NetworkText.FromLiteral(""), 0, x2, y2, 0f, 0); }
                                }
                            }
                            if (canExplode)
                            {
                                for (int x3 = x2 - 1; x3 <= x2 + 1; x3++)
                                {
                                    for (int y3 = y2 - 1; y3 <= y2 + 1; y3++)
                                    {
                                        if (Main.tile[x3, y3] != null && Main.tile[x3, y3].WallType > 0 && updateWalls)
                                        {
                                            ...
                                            if(sync && Main.tile[x3, y3].WallType == 0 && Main.netMode != NetmodeID.SinglePlayer) { NetMessage.SendData(BaseConstants.NET_TILE_UPDATE, -1, -1, NetworkText.FromLiteral(""), 2, x3, y3, 0f, 0); }
                                        }
                                    }
                                }
                            }
        ...
        */       
        ...
        public static void PlayTileHitSound(float x, float y, int tileType)
        {
            ...
            if (tileType >= 0 && TileLoader.GetTile(tileType) != null)
            {
                ...
                if (tile.HitSound.HasValue)
                {
                    SoundEngine.PlaySound(tile.HitSound.Value, new Vector2(x, y));
                }
            }
