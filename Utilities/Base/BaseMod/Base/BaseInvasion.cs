namespace CSkies.Utilities.Base.BaseMod.Base
{
    /*public class BaseInvasion
	{
		...
		public void OnWorldUpdate()
		{
			...
			undeadipedeCount = CSkies.BaseAI.GetNPCs(default(Vector2), GRealm.UNDEADIPEDE_TYPES[0], -1, null).Length + CSkies.BaseAI.GetNPCs(default(Vector2), GRealm.UNDEADIPEDE_TYPES[1], -1, null).Length;	
		...
		public void HandleHardMobSpawn(int type)
		{
            if (Main.netMode != 1 && IsHardMob(type))
			{
				...
				if(chat != null) CSkies.BaseUtility.Chat(chat, CSkies.BaseConstants.CHATCOLOR_PURPLE);				
			}	
		}
		...
		public static IDictionary<int, float> SpawnPool(IDictionary<int, float> startingPool, int x, int y, Player player)
		{
			...
			}else //normal zombies
			{
				...
				if (ZONEDEV || CSkies.BaseTile.LiquidCount(x, y, 25, 0) > 120) //floaty zombie
				{
