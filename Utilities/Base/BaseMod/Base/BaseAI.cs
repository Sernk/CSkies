    ...
    public class BaseAI
    {
			...
			if (isHead)
			{
				...
				}else
				{
					...
					if (newPointDist.Length() > 100f)
					{
						...
						{
							p.velocity.X = p.velocity.X + velocityScalarIdle * Math.Sign(plrCenter.X - p.Center.X);
						}
						if (Math.Abs(plrCenter.Y - p.Center.Y) > 10f)
						{
							p.velocity.Y = p.velocity.Y + velocityScalarIdle * Math.Sign(plrCenter.Y - p.Center.Y);
						}
					}
		...
		public static void AIProjSpaceOctopus(Projectile projectile, ref float[] ai, int parentNPCType, int fireProjType = -1, float shootVelocity = 16f, float hoverTime = 210f, float xMult = 0.15f, float yMult = 0.075f, int fireDmg = -1, bool useParentTarget = true, bool noParentHover = false, Action<int, Projectile> SpawnDust = null)
		{
			...
			if (ai[0] == hoverTime)
			{
				...
				}else
				{
					...
					{
						Projectile.NewProjectile(projectile.GetSource_FromThis(), projectile.Center.X, projectile.Center.Y, distVecNormal.X * shootVelocity, distVecNormal.Y * shootVelocity, fireProjType, fireDmg, 1f, Main.myPlayer, 0f, 0f);
					}
					ai[0] = 0f;
				}
			}
		...
		*/
            ...
            if (p.ai[0] == 0f)
            {
                ...
                if (p.alpha <= 0)
                {
                    ...
                    if (p.ai[1] < length && Main.myPlayer == p.owner)
                    {
                        ...
                        int id = Projectile.NewProjectile(p.GetSource_FromThis(), p.Center.X + p.velocity.X, p.Center.Y + p.velocity.Y, rotVec.X, rotVec.Y, p.type, p.damage, p.knockBack, p.owner);
		...
		public static void AIFriendly(NPC npc, ref float[] ai, float moveInterval = 0.07f, float maxSpeed = 1f, bool critter = false, bool? seekHome = null, bool canTeleportHome = true, bool canFindHouse = true, bool canOpenDoors = true)
		{
			...
			if ((npc.type >= NPCID.Count && npc.homeTileX == -1 && npc.homeTileY == -1) || (npc.homeTileX == ushort.MaxValue && npc.homeTileY == ushort.MaxValue))
			{
			...
			}else
			{
				...
				if (npc.closeDoor && ((npc.Center.X / 16f > npc.doorX + 2) || (npc.Center.X / 16f < npc.doorX - 2)))
				{
					...
					{
						npc.closeDoor = false;
						NetMessage.SendData(MessageID.ToggleDoorState, -1, -1, NetworkText.FromLiteral(""), 1, npc.doorX, npc.doorY, npc.direction, 0);
					}
				...
				if (canOpenDoors && Main.tile[tileX2, tileY2 - 2].HasUnactuatedTile && Main.tile[tileX2, tileY2 - 2].TileType == 10 && (Main.rand.Next(10) == 0 || seekHouse))
				{
					...
					if (WorldGen.OpenDoor(tileX2, tileY2 - 2, npc.direction))
					{
						...
						npc.doorX = tileX2;
						npc.doorY = tileY2 - 2;
						NetMessage.SendData(MessageID.ToggleDoorState, -1, -1, NetworkText.FromLiteral(""), 0, tileX2, tileY2 - 2, npc.direction, 0);
						npc.netUpdate = true;
						ai[1] += 80f;
		...
		public static void AIPlant(NPC npc, ref float[] ai, bool checkTilePoint = true, Vector2 endPoint = default, bool isTilePos = true, float vineLength = 150f, float vineLengthLong = 200f, int vineTimeExtend = 300, int vineTimeMax = 450, float moveInterval = 0.035f, float speedMax = 2f, Vector2 targetOffset = default)
        {
            ...
            if (checkTilePoint)
            {
                ...
                {
                    if(npc.DeathSound != null) SoundEngine.PlaySound(npc.DeathSound, npc.position);
                    npc.life = -1;
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
				...
				if (!singlePiece && split)
                {
                    ...
                    if (isBody && !Main.npc[(int)npc.ai[1]].active) //if the body was just split, turn it into a head
                    {
                        ...
                        NPCSpawnParams spawnParams = default; // Create a default NPCSpawnParams instance
                        npc.SetDefaults(npc.type, spawnParams); // npc.SetDefaults(npc.type, -1f);
					...
					if (isBody && !Main.npc[(int)npc.ai[0]].active) //if the body was just split, turn it into a tail
					{
                        ...
                        NPCSpawnParams spawnParams = default;
                        npc.SetDefaults(npc.type, spawnParams); // npc.SetDefaults(npc.type, -1f);
                        npc.life = (int)(npc.lifeMax * lifePercent);
						npc.ai[1] = lastPiece;
						npc.TargetClosest(true);
        ...
        public static void AIFish(NPC npc, ref float[] ai, bool hostile = false, bool ignoreNonWetPlayer = true, bool ignoreWater = false, float velMaxX = 3f, float velMaxY = 2f)
        {
            ...
            if (ignoreWater || npc.wet)//if wet or ignore water is true...
            {
                ...
                if (hasTarget)
                {
                    ...
                    npc.velocity.Y = npc.velocity.Y + npc.directionY * 0.1f;
                ...
                if (Main.tile[tileX, tileY - 1].HasUnactuatedTile && Main.tile[tileX, tileY - 1].TileType == 10)
                {
                    ...
                    if (doorCounter >= doorCounterMax)
                    {
                        ...
                        if (attemptOpenDoor && Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            {
                                NetMessage.SendData(MessageID.ToggleDoorState, -1, -1, NetworkText.FromLiteral(""), 0, tileX, tileY, npc.direction, 0);
                            }
                        }
                    }
                    return true;
        ...
        public static int DropItemBossBag(Player player, Entity codable, int type, int amt, int maxStack, float chance, bool clusterItem = false)
        {
			if(player != null)
			{
				if ((float)Main.rand.NextDouble() <= chance) player.QuickSpawnItem(player.GetSource_FromThis(), type, amt);
				return -2;
			}else
			{
            ...
            if ((sync || Main.netMode != NetmodeID.MultiplayerClient) && (float)Main.rand.NextDouble() <= chance)
            {
                if (clusterItem)
                {
                    ...
                    while (stackCount != amt)
                    {
                        ...
                        {
                            itemID = Item.NewItem(codable.GetSource_FromThis(), (int)codable.position.X, (int)codable.position.Y, codable.width, codable.height, type, stackCount2, false, 0);
                ...
                }else
                {
                    ...
                    {
                        count++;
                        itemID = Item.NewItem(codable.GetSource_FromThis(),(int)codable.position.X, (int)codable.position.Y, codable.width, codable.height, type, 1, false, 0);
						if(sync) NetMessage.SendData(MessageID.SyncItem, -1, -1, null, itemID, 0f, 0f, 0f, 0, 0, 0);						
                    }
                }
            }
        ...
        public static void DamagePlayer(Player player, int dmgAmt, float knockback, int hitDirection, Entity damager, bool dmgVariation = true, bool hitThroughDefense = false, int critChance = 0, float critMult = 1f)
        {
            ...
            if (damager == null)
            {
                ...
                int dmgDealt = (int)player.Hurt(PlayerDeathReason.ByOther(-1), parsedDamage, hitDirection, false, false);
            ...
            if (damager is Player)
            {
				...
				//int dmgDealt = (int)player.Hurt(PlayerDeathReason.PlayerDeathReason(subPlayer.whoAmI), parsedDamage, hitDirection, true, false, false, 0);
            ...
            if (damager is Projectile)
            {
                ...
                if(p.friendly)
                {
                    ...
                    int parsedDamage = dmgAmt; if (dmgVariation) { parsedDamage = Main.DamageVar(dmgAmt); }
                    int dmgDealt = (int)player.Hurt(PlayerDeathReason.ByProjectile(p.owner, p.whoAmI), parsedDamage, hitDirection, true, false);
                ...
                if(p.hostile)
                {
					...
					int parsedDamage = dmgAmt; if (dmgVariation) { parsedDamage = Main.DamageVar(dmgAmt); }
                    int dmgDealt = (int)player.Hurt(PlayerDeathReason.ByProjectile(-1, p.whoAmI), parsedDamage, hitDirection, false, false);
            ...
            if (damager is NPC)
            {
                ...
                int parsedDamage = dmgAmt; if (dmgVariation){ parsedDamage = Main.DamageVar(dmgAmt); }
                int dmgDealt = (int)player.Hurt(PlayerDeathReason.ByNPC(npc.whoAmI), parsedDamage, hitDirection, false, false);
        ...
        public static void DamageNPC(NPC npc, int dmgAmt, float knockback, int hitDirection, Entity damager, bool dmgVariation = true, bool hitThroughDefense = false)
        {
            ...
            if (damager is Projectile)
            {
                ...
                if (p.owner == Main.myPlayer)
                {
                    ...
                    int resultDmg = (int)npc.StrikeNPC(hitInfo, false, false);
            ...
            if (damager is Player)
            {
                ...
                if (player.whoAmI == Main.myPlayer)
                {
                    ...
                    int dmgDealt = (int)npc.StrikeNPC(hitInfo, false, false);
         ...
         */
		public static void SpawnSmoke(Projectile projectile, Vector2 start, float width, float height, int loopAmount = 2, float scale = 1f)
        {
            ...
            for (int m = 0; m < loopAmount; m++)
            {
                ...
                int goreID = Gore.NewGore(source, gorePos, velocityDefault, Main.rand.Next(61, 64), 1f);
                ...
                goreID = Gore.NewGore(source, gorePos, velocityDefault, Main.rand.Next(61, 64), 1f);
            ...
            if (properSide)
            {
                ...
                if (delayTimer <= 0)
                {
                    if (!checkCanHit || Collision.CanHit(codable.position, codable.width, codable.height, position, width, height))
                    {
                        ...
                        pID = FireProjectile(targetCenter, codable as NPC, projType, damage, 0f, speed);
                    }
        ...
        public static int FireProjectile(Vector2 fireTarget, NPC npc, int projectileType, int damage, float knockback, float speedScalar = 1.0F, int soundGroup = 0, int sound = -1, int hostility = 0, int owner = -1)
        {
            ...
            {
                //SoundEngine.PlaySound(soundGroup, npc.position, sound);
            }
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                int projectileID = FireProjectile(npc, fireTarget, npc.Center, projectileType, damage, knockback, speedScalar, hostility, owner);
                npc.netUpdate = true;
                return projectileID;
            }
        ...
        public static int FireProjectile(Vector2 fireTarget, Projectile p, int projectileType, int damage, float knockback, float speedScalar = 1.0F, int soundGroup = 0, int sound = -1, int hostility = 0, int owner = -1)
        {
            ...
            if (p.owner == Main.myPlayer)
            {
                return FireProjectile(fireTarget, p, projectileType, damage, knockback, speedScalar, hostility, owner);
            }
         ...
         */
        public static int FireProjectile(NPC npc, Vector2 fireTarget, Vector2 position, int projectileType, int damage, float knockback, float speedScalar = 1f, int hostility = 0, int owner = -1, Vector2 targetOffset = default)
        {
            ...
            rotVec -= position;
            int projectileID = Projectile.NewProjectile(npc.GetSource_FromThis(), position.X, position.Y, rotVec.X, rotVec.Y, projectileType, damage, knockback, (owner != -1 ? owner : Main.myPlayer));
