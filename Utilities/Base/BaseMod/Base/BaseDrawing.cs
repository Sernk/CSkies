...
namespace CSkies.Utilities.Base.BaseMod.Base
{
    ...
    public class BaseDrawing
    {
		...
		public static void DrawInvasionProgressBar(SpriteBatch sb, int progress, int progressMax, bool forceDisplay, ref int displayCount, ref float displayAlpha, Texture2D iconTex, string displayText, string percentText = null, Color backgroundColor = default, Vector2 offset = default)
		{
            ...
            if (progressMax != 0)
			{
				...
				Main.spriteBatch.Draw(TextureAssets.MagicPixel.Value, vector4, new Microsoft.Xna.Framework.Rectangle?(new Rectangle(0, 0, 1, 1)), new Color(255, 241, 51) * displayAlpha, 0f, new Vector2(1f, 0.5f), new Vector2(scalarX * progressPercent, scalarY), SpriteEffects.None, 0f);
				Main.spriteBatch.Draw(TextureAssets.MagicPixel.Value, vector4, new Microsoft.Xna.Framework.Rectangle?(new Rectangle(0, 0, 1, 1)), new Color(255, 165, 0, 127) * displayAlpha, 0f, new Vector2(1f, 0.5f), new Vector2(2f, scalarY), SpriteEffects.None, 0f);
				Main.spriteBatch.Draw(TextureAssets.MagicPixel.Value, vector4, new Microsoft.Xna.Framework.Rectangle?(new Rectangle(0, 0, 1, 1)), Color.Black * displayAlpha, 0f, new Vector2(0f, 0.5f), new Vector2(scalarX * (1f - progressPercent), scalarY), SpriteEffects.None, 0f);
			}
        ...
        {
            // Adjusted to use the correct property for player textures
            switch (name)
            {
                case "Head": return TextureAssets.Players[skinVariant, 0].Value;
                case "EyeWhite": return TextureAssets.Players[skinVariant, 1].Value;
                case "Eye": return TextureAssets.Players[skinVariant, 2].Value;
                case "Body": return (male ? TextureAssets.Players[skinVariant, 4].Value : TextureAssets.Players[skinVariant, 6].Value);
                case "Hand": return TextureAssets.Players[skinVariant, 5].Value;
                case "Arms": return TextureAssets.Players[skinVariant, 7].Value;
                case "Legs": return TextureAssets.Players[skinVariant, 10].Value;
            }
            return null;
        }
        ...
        public static void AddPlayerHeadLayer(List<PlayerDrawLayer> list, PlayerDrawLayer layer, PlayerDrawLayer parent, bool first)
        {
            int insertAt = -1;
            for (int m = 0; m < list.Count; m++)
            {
         ...
         */
        public static bool IsNormalDrawPass(Player player, PlayerDrawSet pdi = default)
        {
            return player.ghostFade == 0f && pdi.shadow == 0f;
        }
        ...
        {
            return p.GetImmuneAlpha(BuffEffects(p, GetLightColor(position != null ? (Vector2)position : p.Center), (shadowOverride != 0f ? shadowOverride : p.ghostFade), effects, p.poisoned, p.onFire, p.onFire2, false, p.noItems, p.blind, p.bleed, p.venom, false, p.ichor, p.onFrostBurn, p.burned, p.honey, p.dripping, p.drippingSlime, p.loveStruck, p.stinky), p.ghostFade);
        }
        ...
        public static Color BuffEffects(Entity codable, Color lightColor, float shadow = 0f, bool effects = true, bool poisoned = false, bool onFire = false, bool onFire2 = false, bool hunter = false, bool noItems = false, bool blind = false, bool bleed = false, bool venom = false, bool midas = false, bool ichor = false, bool onFrostBurn = false, bool burned = false, bool honey = false, bool dripping = false, bool drippingSlime = false, bool loveStruck = false, bool stinky = false)
        {
			...
			if (effects && honey && Main.rand.Next(30) == 0)
			{
				...
				// if (codable is Player) player.playerDrawDust.Add(dustID); Я не знаю как это исправить
            }
            if (poisoned)
            {
		...
		public static void DrawTileTexture(SpriteBatch sb, Texture2D texture, int x, int y, int fwidth = 16, int fheight = 16, int frameX = 0, int frameY = 0, bool slopeDraw = true, bool flipTex = false, bool ignoreHalfBricks = false, bool? overrideHalfBrick = null, Func<Color, Color> overrideColor = null, Vector2 offset = default)
		{
			...
			if (tile.IsActuated){ color = Lighting.GetColor(x, y); }
			...
			if(!ignoreHalfBricks && Main.tileSolid[tile.TileType] && !halfBrick && (Main.tile[x - 1, y].IsHalfBlock || Main.tile[x + 1, y].IsHalfBlock))
			{
			...
			if (Lighting.LegacyEngine.Mode < 2 && Main.tileSolid[tile.TileType] && !halfBrick && !tile.IsActuated)
			{
			...
			if (halfBrickOffset == 8 && (!Main.tile[x, y + 1].HasTile || !Main.tileSolid[Main.tile[x, y + 1].TileType] || Main.tile[x, y + 1].IsHalfBlock))
			{
		...
		public static void DrawWallTexture(SpriteBatch sb, Texture2D texture, int x, int y, bool drawOutline = false, Func<Color, Color> overrideColor = null, Vector2 offset = default)
		{
			...
			int frameOffsetY = (hasWall ? Main.wallFrame[tile.WallType] * 180 : 0);
		...
		public static void DrawWallTexture(SpriteBatch sb, Texture2D texture, int x, int y, int wallFrameX, int wallFrameY, int frameOffsetY, bool drawOutline = false, Func<Color, Color> overrideColor = null, Vector2 offset = default)
		{
			...
			if (Lighting.LegacyEngine.Mode == 2){ tileColorFloat = (Main.tileColor.R - 12) / 255f; }else
			if (Lighting.LegacyEngine.Mode == 3){ tileColorFloat = (tileColor - 12) / 255f; }
			...
			{
				bool outlineLeft = Main.tile[x - 1, y].WallType > 0 && Main.wallBlend[Main.tile[x - 1, y].WallType] != Main.wallBlend[Main.tile[x, y].WallType];
				bool outlineRight = Main.tile[x + 1, y].WallType > 0 && Main.wallBlend[Main.tile[x + 1, y].WallType] != Main.wallBlend[Main.tile[x, y].WallType];
				bool outlineUp = Main.tile[x, y - 1].WallType > 0 && Main.wallBlend[Main.tile[x, y - 1].WallType] != Main.wallBlend[Main.tile[x, y].WallType];
				bool outlineDown = Main.tile[x, y + 1].WallType > 0 && Main.wallBlend[Main.tile[x, y + 1].WallType] != Main.wallBlend[Main.tile[x, y].WallType];
    ...
    public class AmmoSlotRender
    {
		...
		public virtual void Draw(SpriteBatch sb, Color color, Item item, Vector2 pos, float sc) 
        {
			...
			ChatManager.DrawColorCodedStringWithShadow(sb, FontAssets.ItemStack.Value, s, pos + new Vector2(10f * sc, 32f * sc), color, 0f, default, new Vector2(sc *= 0.8f), -1f, 0.8f);
        }
    }
	...
	public class BaseArmorData : ArmorShaderData
	{
		...
		public override void Apply(Entity entity, DrawData? drawData = null)
		{
			try
			{
				...
				if(ent != null)
				{
					...
					if(ent is NPC)
					{
                        ...
                        if (((NPC)ent).ModNPC != null && ((NPC)ent).ModNPC.GetType().Name == "ParentNPC")
                        {
                            Shader.Parameters["uFrame"].SetValue(((dynamic)((NPC)ent).ModNPC).GetFrameV4());
                        }
                        else
                        {
					...
					{					
                        ...
                        Shader.Parameters["uTexSize"].SetValue(v4);
                        if (proj.ModProjectile != null && proj.ModProjectile.GetType().Name == "ParentProjectile")
                        {
                            Shader.Parameters["uFrame"].SetValue((Vector4)proj.ModProjectile.GetType().GetMethod("GetFrameV4").Invoke(proj.ModProjectile, null));
                        }
                        else
                        {
                            Shader.Parameters["uFrame"].SetValue(v4_2);
					...
					if(ent is Player)
					{
                        Vector4 v4 = new Vector4(0, 0, TextureAssets.Players[0, 0].Value.Width, TextureAssets.Players[0, 0].Value.Height);
			...
			{
				BaseUtility.LogFancy("CSkies~ BASE ARMOR ERROR:", e);
			}
		}
