    ...
    public class BaseUtility
    {
			...
			}else
			{
				...
				if(chest >= 0)
				{
					...
					}else
					{
						...
						player.chestX = left;
						player.chestY = top;
                        SoundEngine.PlaySound(player.chest < 0 ? SoundID.MenuClose : SoundID.MenuOpen);
                    }
                    Recipe.FindRecipes();
				}
			}			
        ...
        public static NPC NPCByName(string n)
        {
            if (n.Contains(":"))
            {
                ...
                string n2 = n.Split(':')[1];
                Mod mod = ModLoader.GetMod(mName);
                if (mod.TryFind(n2, out ModNPC modNPC))
                {
                    return modNPC.NPC;
                }
            }
            else
            {
                ...
                foreach (string name in modNames)
                {
                    ...
                    if (mod.TryFind(n, out ModNPC modNPC))
                    {
                        return modNPC.NPC;
                    }
                }
        ...
        public static Item ItemByName(string n)
        {
            if (n.Contains(":"))
            {
                ...
                string n2 = n.Split(':')[1];
                Mod mod = ModLoader.GetMod(mName);
                if (mod.TryFind(n2, out ModItem modItem))
                {
                    return modItem.Item;
                }
            }
            else
            {
                ...
                {
                    Mod mod = ModLoader.GetMod(name);
                    if (mod.TryFind(n, out ModItem modItem))
                    {
                        return modItem.Item;
                    }
                }
        ...
        public static Projectile ProjByName(string n)
        {
            if (n.Contains(":"))
            {
                ...
                string n2 = n.Split(':')[1];
                Mod mod = ModLoader.GetMod(mName);
                if (mod.TryFind(n2, out ModProjectile modProjectile))
                {
                    return modProjectile.Projectile;
                }
            }
            else
            {
                ...
                {
                    Mod mod = ModLoader.GetMod(name);
                    if (mod.TryFind(n, out ModProjectile modProjectile))
                    {
                        return modProjectile.Projectile;
                    }
                }
        ...
        public static ModTile TileByName(string n)
        {
            if (n.Contains(":"))
            {
                ...
                string n2 = n.Split(':')[1];
                Mod mod = ModLoader.GetMod(mName);
                if (mod.TryFind(n2, out ModTile modTile))
                {
                    return modTile;
                }
            }
            else
            {
                ...
                {
                    Mod mod = ModLoader.GetMod(name);
                    if (mod.TryFind(n, out ModTile modTile))
                    {
                        return modTile;
                    }
                }
