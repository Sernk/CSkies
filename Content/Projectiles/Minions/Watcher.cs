...
namespace CSkies.Content.Projectiles.Minions
{
    public class Watcher : ModProjectile
	{
        ...
        {
            bool flag64 = Projectile.type == ModContent.ProjectileType<Watcher>();
            Player player = Main.player[Projectile.owner];
            CPlayer modPlayer = player.GetModPlayer<CPlayer>();
            player.AddBuff(ModContent.BuffType<Content.Buffs.Watcher>(), 3600);
            if (flag64)
            {
                if (player.dead)
                {
                    modPlayer.Watcher = false;
                }
            ...
            {
                bool flag23 = Main.projectile[num638].type == ModContent.ProjectileType<FlameBlast>();
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            BaseDrawing.DrawAfterimage(sb, Tex, 0, Projectile.position, Projectile.width, Projectile.height, Projectile.oldPos, 1, 0, Projectile.spriteDirection, 4, frame, .8f, 1, 4, true, 0, 0, Projectile.GetAlpha(Color.White * 0.8f));
